...
def decompose_mix(mixarr, avg_tk=False):
    ...
    """
    ret = np.zeros((
        2, LT_CHECK, LT_CHECK), np.complex) if not avg_tk else np.zeros((
...
def decompose(array, ncontract, avg_tk=False, tstep=1):
                ...
                for conidx in range(ncontract):
                    ...
                    assert idx == idx_prev+1,\
                        "Index is running too fast:"+str(
                            idx)+","+str(idx_prev)
                    if avg_tk and timek == 0:
                        ret[conidx, gcombidx, tdis] = array[idx]
                    elif avg_tk and timek != 0:
                        ret[conidx, gcombidx, tdis] += array[idx]
                    else:
...
# inline int map(const int tk, const int t_dis,
# const int con_idx, const int gcombidx, const int thread) const{
#  return con_idx + ncontract*(
# gcombidx + 4*( t_dis + Lt*( tk + Lt*thread) ) );
# }
