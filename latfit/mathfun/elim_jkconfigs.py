...
def elim_jkconfigs(jkblk):
    if not JACKKNIFE:
        ...
        sys.exit(1)
    if not type(ELIM_CONF_LIST) is list or not type(ELIM_CONF_LIST) is np.ndarray or not type(ELIM_CONF_LIST[0]) is int:
        print("Not eliminating any configs because of misconfigured list of configs to elimiante.")
        print("Check config and rerun.")
        sys.exit(1)
    else:
        n=len(jkblk)
        k=len(ELIM_CONF_LIST)
        if k==0:
            new_jkblk=jkblk
        else:
            skip_sum=np.sum([jkblk[skip] for skip in ELIM_CONF_LIST],axis=0)
            sum_blk=np.sum(jkblk,axis=0)
            new_jkblk=1.0/(n-1-k)*((n-1)*(np.delete(jkblk,ELIM_CONF_LIST,axis=0)+skip_sum)-k*sum_blk)
    return new_jkblk
